...
async function compileClassicLevel(
  ...
  const packDoc = applyHierarchy(async (doc: Document, collection: string) => {
    ...
    await mapHierarchy(value, collection, async (d: Document) => d._id)
    batch.put(key, value)
  })
...
type HierarchyMapCallback = (entry: object, collection: string) => Promise<unknown>
 ...
 */
async function mapHierarchy(doc: object & Record<string, unknown>, collection: string, fn: HierarchyMapCallback) {
  for (const [embeddedCollectionName, type] of Object.entries(HIERARCHY[collection] ?? {})) {
    const embeddedValue = doc[embeddedCollectionName]
